...
static void btree_write_node(struct btree_t* btree, struct btree_node_t* node) {
}

static unsigned char btree_delete_node(struct btree_t* btree, struct btree_node_t* node) {
	return 0;
}

static struct btree_node_t* btree_load_node(struct btree_t* btree, unsigned int childId) {
	return 0;
}
...
static unsigned char btree_node_merge_children(struct btree_node_t* self, struct btree_t* btree, unsigned int childrenIndex, struct btree_node_t* child, struct btree_node_t* sibling) {
	...
	if (!child->leaf) {
		...
		int firstKeyOfSibling = sibling->keys[0];
		void* firstValueOfSibling = sibling->values[0];
		memcpy(sibling->keys, sibling->keys + 1, (sibling->keySize - 1) * sizeof(unsigned int));
		memcpy(sibling->values, sibling->values + 1, (sibling->keySize - 1) * sizeof(void*));
		child->keys[child->keySize] = firstKeyOfSibling;
		child->values[child->keySize] = firstValueOfSibling;
		child->keySize++;
		if (!child->leaf) {
			int firstChildIdOfSibling = sibling->children[0];
			memcpy(sibling->children, sibling->children + 1, (sibling->childrenSize - 1) * sizeof(unsigned int));
	...
	if (!btree_delete_node(btree, sibling)) {
		return 1;
	}
	return 0;
}
...
static struct btree_node_t* btree_node_create_right_sibling(struct btree_node_t* self, struct btree_t* btree) {
...
static void btree_node_split_child(struct btree_node_t* self, struct btree_t* btree, int i, struct btree_node_t* child) {
	struct btree_node_t* sibling = btree_node_create_right_sibling(child, btree);
	unsigned int key = child->keys[HALF_MAX - 1];
	memcpy(child->keys + HALF_MAX - 1, child->keys + HALF_MAX, (child->keySize - HALF_MAX) * sizeof(unsigned int));
	memcpy(child->values + HALF_MAX - 1, child->values + HALF_MAX, (child->keySize - HALF_MAX) * sizeof(void*));
	child->keySize--;
	btree_node_add_locally(self, key);
	btree_write_node(btree, child);
	self->children[i + 1] = sibling->id;
}
	...
	while (!node->leaf) {
		...
		if (i == d) {
			...
			if (btree_node_is_full(child)) {
				btree_node_split_child(node, btree, i, child);
			} else {
